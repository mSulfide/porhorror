...
```
**Примечание** Group: Представляет информацию о группе пользователей
...
```
{
    ...
    creatorId: идентификатор создателя группы
}
```
**Успешный ответ**
```
...
```
{
    groupId - идентификатор группы, которую нужно удалить
}
```
**Успешный ответ**
```
    Answer<true>
    Answer<{
        "deleteGroup": boolean; //Возвращает true, если операция была успешной
                                //Возвращает false, если произошла ошибка 
    }>
```
**Ошибки**
