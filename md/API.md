...
* `242` - отсутствует токен
* `808` - статус уже "ready"


### 4.25. addLotComment
Добавить комментарий к лоту
**Параметры**
```
{
    "token": "string",  // токен пользователя
    "lotId": "integer",  // идентификатор лота
...
* `705` - пользователь не найден
* `807` - лот не найден
* `242` - отсутствует токен, идентификатор лота или текст комментария
### 4.26. deleteLot
Удаление лота
...
```
{
    ...
    "lotId": "integer"    // идентификатор лота, который нужно удалить
}
```
...
```
**Ошибки**
* `806` - пользователь не является владельцем лота
* `807` - лот не найден
* `242` - отсутствует токен, идентификатор лота или текст комментария

### 4.27. createLot
Создание нового лота для обмена

**Параметры**
```
{
    "token": "string",  // токен авторизации пользователя
    "itemsToGive": ["integer"],  // массив идентификаторов предметов, которые пользователь хочет отдать
    "itemsToReceive": ["integer"]  // массив идентификаторов предметов, которые пользователь хочет получить
}
```
**Успешный ответ**
```
Answer<{
    lotId: number;  // идентификатор созданного лота
}>
```
**Ошибки**

* `810` - недостаточно предметов в инвентаре
* `811` - не удалось создать лот
* `812` - ошибка добавления предметов в лот
* `242` - отсутствуют необходимые параметры (токен, itemsToGive или itemsToReceive)

### 4.28. addLotItem
Добавление предмета в существующий лот
...
```
{
    ...
    "type": "string"      // тип добавляемого предмета ('give' или 'receive')
}
```
**Успешный ответ**
```
...
* `807` - лот не найден или не принадлежит пользователю
* `810 `- недостаточно предметов в инвентаре (если type = 'give')
* `242` - отсутствуют необходимые параметры (токен, lotId, itemId или type)

### 4.27. removeItemFromLot

Удаление предмета из лота
...
```
{
    "token": "string",  // токен авторизации пользователя
    "lotId": "integer",  // идентификатор лота
    "itemId": "integer"  // идентификатор предмета, который нужно удалить
}
```
**Успешный ответ**
```
...
* `807`- лот не найден или не принадлежит пользователю
* `813` - предмет не найден в указанном лоте
* `242` - отсутствуют необходимые параметры (токен, lotId или itemId)
### 4.28. takeOffItem
Снятие предмета из кармана ("pocket") и перемещение его обратно в основной инвентарь.

**Параметры**
```
{
    "token": "string",  // токен авторизации пользователя
    "slotId": "integer" // идентификатор слота предмета, который нужно снять
}
```
**Успешный ответ**
...
* `820` - слот не найден или не принадлежит пользователю
* `821` - предмет уже находится в основном инвентаре
* `242` - отсутствуют необходимые параметры (токен или slotId)
**Пример запроса**
...
```
**Пример ошибочного ответа (слот не найден)**
...
```
{
    ...
    "error": {
        "code": 820,
        "text": "Слот не найден или не принадлежит пользователю"
    }
}
```
 ### 4.29. equipItem
...
{
    "token": "string",   // токен авторизации пользователя
    "slotId": "integer"  // идентификатор слота предмета, который нужно надеть
}
```
**Успешный ответ**
```
...
* `820` - слот не найден или не принадлежит пользователю
* `821` - предмет уже находится в кармане (pocket)
* `830` - нет свободного места в кармане (максимум inventory_max_count предметов)
* `242` - отсутствуют необходимые параметры (токен или slotId)